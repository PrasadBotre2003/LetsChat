...
fun ChatListScreen(navcontroller: NavController, vm: LCViewModel) {
    ...
    if (inProgress.value) {
        ...
        var onFabClick: () -> Unit = { showDialog.value = true }
        var onDismiss: () -> Unit = { showDialog.value = false }
        var onAddChat: (String) -> Unit = {
            vm.onAddChat(it)//it
        ...
        Scaffold(
            floatingActionButton = {
                ...
                FAB(showDialog = showDialog.value, onFabClick =
                 onFabClick, onDismiss =  onDismiss,
                    onAddChat = onAddChat)
                            ...
                            items(chats) { chat ->
                                ...
                                CommonRow(imageUrl = chatUser.imageUrl, name = chatUser.name) {
                                    chat.chatId?.let {
                                        navigateto(
                                            navcontroller,
                                            DestinationScreen.SingelChat.createRoute(id = it)
                                        )
